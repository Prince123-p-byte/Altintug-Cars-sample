...
// ============ REVIEWS & RATINGS ============
// No login — visitors leave a name + email with their review. Reviews are
// stored locally (no backend) under 'altintug_reviews', same pattern as
// admin-uploaded vehicles, and merged with a few seed reviews so the
// section isn't empty on a fresh browser.
...
// Lets people type freely (e.g. "tayota rav4") and still match "Toyota RAV4",
// the same way Google tolerates misspellings.
function normalizeSearchText(str){
  return String(str || '')
    .toLowerCase()
    .replace(/ı/g,'i').replace(/İ/g,'i').replace(/ğ/g,'g').replace(/ş/g,'s')
...
function wordFuzzyMatch(word, target){
...
function vehicleSearchWords(v){
  const parts = [v.name, v.make, getModelLabel(v), v.fuel, v.color, String(v.year), ...(v.cats || [])];
  return normalizeSearchText(parts.join(' ')).split(' ').filter(Boolean);
}

function vehicleMatchesQuery(v, query){
  const q = normalizeSearchText(query);
  if(!q) return true;
  const queryWords = q.split(' ').filter(Boolean);
  const targetWords = vehicleSearchWords(v);
  return queryWords.every(qw => targetWords.some(tw => wordFuzzyMatch(qw, tw)));
}
...
function runSearch(){
  ...
  let result = vehicles.filter(v=>{
    ...
    if(query && !vehicleMatchesQuery(v, query)) return false;
    return true;
  });
...
// ============ TYPED SEARCH: LIVE AUTOCOMPLETE ============
const fQueryInput = document.getElementById('fQuery');
const searchSuggestEl = document.getElementById('searchSuggest');
let suggestTimer;

function renderSuggestions(){
  const q = fQueryInput.value.trim();
  if(!q){ hideSuggestions(); return; }
  const matches = vehicles.filter(v=>vehicleMatchesQuery(v, q)).slice(0,6);
  searchSuggestEl.innerHTML = matches.length
    ? matches.map(v=>`
        <div class="search-suggest-item" data-id="${v.id}">
          <span>${v.name}</span>
          <b>£${v.price.toLocaleString()}</b>
        </div>`).join('')
    : `<div class="search-suggest-item search-suggest-empty">No matches — try a different spelling</div>`;
  searchSuggestEl.classList.add('show');
}
function hideSuggestions(){ searchSuggestEl.classList.remove('show'); }

fQueryInput.addEventListener('input', ()=>{
  clearTimeout(suggestTimer);
  suggestTimer = setTimeout(renderSuggestions, 120);
});
fQueryInput.addEventListener('focus', renderSuggestions);
fQueryInput.addEventListener('keydown', e=>{
  if(e.key === 'Enter'){ e.preventDefault(); hideSuggestions(); runSearch(); }
  else if(e.key === 'Escape'){ hideSuggestions(); }
});
document.addEventListener('click', e=>{
  if(!e.target.closest('.search-field-text')) hideSuggestions();
});
searchSuggestEl.addEventListener('click', e=>{
  const item = e.target.closest('.search-suggest-item[data-id]');
  if(!item) return;
  const v = vehicles.find(x=>x.id===item.dataset.id);
  if(!v) return;
  fQueryInput.value = v.name;
  hideSuggestions();
  document.getElementById('inventory').scrollIntoView({behavior:'smooth', block:'start'});
...
function galleryHTML(photos){
  ...
  return `
    <div class="modal-hero-gallery" data-gallery-index="0">
      <div class="hero-img-wrap"><img src="${photos[0]}" alt="" data-action="open-lightbox"></div>
      ${arrows}
      ${dots}
    </div>`;
}

// ============ PHOTO LIGHTBOX (large / zoomed view) ============
...
function quickViewHTML(v){
  ...
  return `
    ...
    <div class="modal-actions">
      ...
      <a class="btn-ghost" href="tel:+905488675432">
  ...
  return `
    ...
    <form id="testDriveForm">
      ...
      <div class="form-row">
        ...
        <input id="td-email" type="email" required placeholder="you@example.com">
      </div>
      <div class="form-grid">
        <div class="form-row">
...
modalBody.addEventListener('click', e=>{
  ...
  const lightboxTrigger = e.target.closest('[data-action="open-lightbox"]');
  if(lightboxTrigger){
    const id = modalBody.dataset.currentId;
    const v = vehicles.find(x=>x.id===id);
    const photos = (v && v.photos) || [];
    const wrap = e.target.closest('.modal-hero-gallery');
    const idx = wrap ? Number(wrap.dataset.galleryIndex || 0) : 0;
